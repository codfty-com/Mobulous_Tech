...
try {
  ...
  assert.equal(summary.totalInvestment, 1200);
  ...
  assert.equal(summary.totalProfitLoss, 800);
  assert.equal(summary.totalProfitLossPercentage, 66.67);
  assert.equal(summary.todayChange, null);
  assert.equal(summary.todayChangeStatus, "unavailable");
  assert.equal((await request("PATCH", "/prices", { updates: [{ id, currentPrice: 200, previousClose: -1 }] })).status, 400);
  assert.equal((await request("PATCH", "/prices", { updates: [{ id, currentPrice: 200, previousClose: 190 }] })).status, 200);
  const dailyHoldings = (await request("GET", "/holdings")).body;
  assert.equal(dailyHoldings.data[0].todayPriceChange, 10);
  assert.equal(dailyHoldings.data[0].todayChange, 100);
  assert.equal(dailyHoldings.data[0].todayChangePercentage, 5.26);
  assert.equal(dailyHoldings.data[0].profitLossStatus, "profit");
  assert.equal(dailyHoldings.data[0].holdingPercentage, 100);
  assert.equal(dailyHoldings.summary.totalHoldingAmount, 2000);
  assert.equal(dailyHoldings.summary.todayChange, 100);
  assert.equal((await request("GET", "/" + id)).body.data.todayChange, 100);
  assert.equal((await request("GET", "/summary")).body.data.overall.todayChangePercentage, 5.26);
  assert.equal((await request("GET", "/net-worth")).body.data.todayChangeStatus, "profit");
  await request("PATCH", "/prices", { updates: [{ id, currentPrice: 110, previousClose: 115 }] });
  const loss = (await request("GET", "/holdings")).body.data[0];
  assert.equal(loss.todayChange, -50);
  assert.equal(loss.todayChangePercentage, -4.35);
  assert.equal(loss.todayChangeStatus, "loss");
  assert.equal(loss.profitLoss, -100);
  assert.equal(loss.profitLossStatus, "loss");
  await request("PATCH", "/prices", { updates: [{ id, currentPrice: 200, previousClose: 190 }] });
